...
const { searchProducts } = require('../../database/search');
 ...
 *   - excludeCategories: comma-separated categories to exclude (optional)
 *   - featured: filter featured products (optional)
 *   - limit: max results (optional)
 */
router.get('/search', async (req, res) => {
  try {
    const { q, category, excludeCategories, featured, limit } = req.query;
    
    const options = {
      query: q,
      category: category,
      excludeCategories: excludeCategories ? excludeCategories.split(',').map(c => c.trim()) : undefined,
      featured: featured === 'true' ? true : undefined,
      limit: limit ? parseInt(limit) : undefined
    };
    
    const products = await searchProducts(options);
