...
// Spreadsheet IDs
const PRODUCTS_DB_ID = process.env.SPREADSHEET_ID; // Jersivo Products Database
const PRODUCTS_ID_MGMT_ID = '1AXTMGhFkPdks0wkTylwmh_sA8G8Q-zWF4QvDfTbfwms'; // Products ID Management
...
const CATEGORY_PREFIX_MAP = {
  ...
  'Special jerseys': 'S',
  'Retro jerseys': 'R',
  'OH - Other Apparel (Hoodie)': 'OH',
  'OJ - Other Apparel (Jacket)': 'OJ',
  'OT - Other Apparel (T-shirt)': 'OT',
  'Gifts': 'G'
};
...
const CATEGORY_SHEET_MAP = {
  ...
  'OH - Other Apparel (Hoodie)': 'Other apparel',
  'OJ - Other Apparel (Jacket)': 'Other apparel',
  'OT - Other Apparel (T-shirt)': 'Other apparel',
  'Gifts': 'Gifts'
};
...
async function generateSKU(category, teamCode) {
  try {
    ...
    // For O category (OH/OJ/OT), check all O variants
    const searchPrefix = prefix.startsWith('O') ? 'O' : prefix;
    ...
    products.forEach(row => {
      ...
      if (sku.startsWith(searchPrefix + '-')) {
...
/**
 * Get display category name (removes OH/OJ/OT prefixes)
 * @param {string} category - Internal category name
 * @returns {string} Display category name
 */
function getDisplayCategory(category) {
  if (category.startsWith('OH') || category.startsWith('OJ') || category.startsWith('OT')) {
